...
import { monthlyCharge, User, Subscription } from "./solution.js";
...
const users2: User[] = [
  {
    id: 1,
    name: "Edna",
    activatedOn: new Date("2019-09-05"),
    deactivatedOn: null,
    customerId: 1,
  },
];
...
describe("monthlyCharge", function () {
  ...
  it("returns 0 immediately if there are no active subscriptions", function () {
    assert.equal(monthlyCharge("2020-02", null, users), 0);
  });
  it("works with a partial user", function () {
    assert.equal(monthlyCharge("2019-10", plan, users2), 4342);
  });
});
